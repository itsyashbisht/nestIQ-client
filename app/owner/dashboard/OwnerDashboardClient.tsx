  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="text-[11px] font-medium uppercase tracking-[0.5px] text-[#a1a4a5]">
          {stat.label}
        </div>
        <div
      ...
      <div className="font-display text-3xl text-white leading-none mb-2">
        {stat.value}
      </div>
      <div
        className={`flex items-center gap-1 text-[12px] ${isPos ? "text-[#11ff99]" : "text-[#ff2047]"}`}
      >
        {isPos ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
        {isPos ? "+" : ""}
        {stat.change}
        {typeof stat.change === "number" && Math.abs(stat.change) < 10
          ? "%"
          : "% this month"}
      </div>
    </motion.div>
...
export default function OwnerDashboardClient() {
  ...
  useEffect(() => {
    ...
    if (isInitialized && user && user.role === "Guest") {
      router.push("/profile");
    }
  }, [user, isInitialized]);
        ...
        >
          <div>
            ...
            <p className="text-[#a1a4a5] text-sm">
              Manage your properties and track revenue
            </p>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-[12px] text-[#a1a4a5]">
                      <span>
                        Occupancy:{" "}
                        <span className="text-white">{hotel.occupancy}%</span>
                      </span>
                      <span className="flex items-center gap-1">
                        <Star size={10} className="text-[#ffc53d]" />{" "}
                        {hotel.rating}
                      </span>
              ...
              <motion.div
                ...
                whileHover={{
                  borderColor: "rgba(255,128,31,0.4)",
            ...
            <div className="rounded-2xl border border-[rgba(214,235,253,0.19)] overflow-hidden">
              ...
              <div className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-4 px-5 py-3 border-b border-[rgba(214,235,253,0.19)] bg-[rgba(255,255,255,0.01)]">
                {["Guest", "Hotel", "Dates", "Amount", "Status"].map((h) => (
                  <div
                    key={h}
                    className="text-[10px] font-semibold uppercase tracking-[0.5px] text-[#a1a4a5]"
                  >
                    {h}
                  </div>
                ))}
              ...
              {MOCK_BOOKINGS.map((b, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[13px] text-[#a1a4a5] truncate">
                      {b.hotel}
                    </div>
